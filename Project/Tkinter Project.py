...
class Application(tk.Tk, Configure_widjets):  # from tkinter module(imported) using inherited (Tk) class
    """Builds GUI."""

    def __init__(self):
        """Create window."""
        tk.Tk.__init__(self)         # need to be written again because of same  __init__ method name of the main class
        self.attributes('-alpha', 1)   # method attributes allows to set new specific values for the properties
        self.attributes('-topmost', True)  # of the tk class as arguments
        self.overrideredirect(True)  # removes window borders
        self.resizable(False, False)  # makes it impossible to change the width and length of the window
        self.title('CPU-RAM usage monitor bar')  # Application name
...
if __name__ == '__main__':
    root = Application()
    root.mainloop()    # launches the window
